...
def generate_data_set(n, m):
    ...
    pickle.dump(labels, open(f"labels_{n}pairs{m}_slices.pickle", 'w'))
    pickle.dump(samples, open(f"samples_{n}pairs{m}_slices.pickle", 'w'))
    return samples, labels


def transform_data(samples, limit):
    """
    ...
    """
    limit = len(samples)
    out_images = np.zeros((limit, 4, 32, 32, 3)).astype(np.float)
    labels = np.zeros((limit, 32, 32, 3)).astype(np.float)
    for j in range(limit):
        ...
        for i in range(0, 4):
            # for each image
            if i == 0:
                out_images[j, i, :, :, 0] = samples[j, i] * 1
                out_images[j, i, :, :, 1] = samples[j, i] * 0
                out_images[j, i, :, :, 2] = samples[j, i] * 0
            elif i == 1:
                out_images[j, i, :, :, 0] = samples[j, i] * 0
                out_images[j, i, :, :, 1] = samples[j, i] * 1
                out_images[j, i, :, :, 2] = samples[j, i] * 0
            elif i == 2:
                out_images[j, i, :, :, 0] = samples[j, i] * 0
                out_images[j, i, :, :, 1] = samples[j, i] * 0
                out_images[j, i, :, :, 2] = samples[j, i] * 1
            else:
                out_images[j, i, :, :, 0] = samples[j, i] * 1
                out_images[j, i, :, :, 1] = samples[j, i] * 1
                out_images[j, i, :, :, 2] = samples[j, i] * 1

            labels[j] += out_images[j, i]

            result_batch = translate_and_rotate(out_images[j])
            out_images[j] = result_batch
...
def rotate_single_data(img, angle):
...
def translate_and_rotate(img_batch):
    for i in range(len(img_batch)):
        ...
        for j in range(3):
            if np.sum(img_batch[i, :, :, j]) == 0:
                continue
            center_of_mass = find_center_of_mass(img_batch[i, :, :, j])
            img_batch[i, :, :, j] = translate_single_data(img_batch[i, :, :, j], center_of_mass)
            img_batch[i, :, :, j] = rotate_single_data(img_batch[i, :, :, j], angle)
    return img_batch
...
def get_data_set():
    ...
    return transform_data(samples, 1000)
...
def train_generator():
    ...
    for j in range(50):
        ...
        for i in range(BATCH_SIZE):
            ...
            samples.append(sample)
        samples = np.array(samples)
        yield transform_data(samples, BATCH_SIZE)
...
class Encoder(Model):
    def __init__(self):
        ...
        self.d1 = Dense(512, activation='relu')
        self.d2 = Dense(6144, activation='relu')
        self.reshape1 = Reshape((8, 8, 96))
        self.deconv1 = Conv2DTranspose(48, 2, activation='relu', strides=2)
        self.deconv2 = Conv2DTranspose(3, 2, activation='relu', strides=2)
        self.reshape2 = Reshape((32, 32, 3))
    ...
    for epoch in range(epochs):
        ...
        if epoch % 20 == 0 and epoch > 0:
            ...
            for j in indices:
                ...
                plt.subplot(2, 3, 2)
                plt.imshow(sample_test[1])
                plt.subplot(2, 3, 3)
                plt.imshow(sample_test[2])
                plt.subplot(2, 3, 4)
                plt.imshow(sample_test[3])
                plt.subplot(2, 3, 5)
